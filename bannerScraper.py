...
class BannerScraper:
    ...
    async def ensure_collection_expiry(self, lifetime):
        self.banner_cache.create_index("datetime", expireAfterSeconds=lifetime)
    ...
    async def get_listings_from_ID(self, course_ID):
        ...
        cached = self.banner_cache.find_one({"year": year, "term": term})

        search_HTML = None

        # If any were found store them in a variable
        if cached is not None:
            print("Got the banner HTML from the cache")
