...
class PeopleScraper:
    ...
    async def ensure_collection_expiry(self, lifetime):
        self.people_cache.create_index("datetime", expireAfterSeconds=lifetime)

    async def get_faculty_staff(self):
        # Try to get the staff data from the cache
        cached = self.people_cache.find_one()
        # If any data was found return it
        if cached is not None:
            print("Got the people data from the cache")
            return cached["data"]
        print("Got the people data by scraping")
