...
class CalendarScraper:
    ...
    async def ensure_collection_expiry(self, lifetime):
        self.calendar_cache.create_index("datetime", expireAfterSeconds=lifetime)

    async def get_calendar_HTML(self):
        # Try to get the HTML data from the cache
        cached = self.calendar_cache.find_one()

        # If any data was found return it
        if cached is not None:
            print("Got the calendar data from cache")
            return cached["data"]
        print("Got the calendar data by scraping")
